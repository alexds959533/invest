...
from web.models import Transactions, Dividends, \
...
class PortfolioDescribe:
    ...
    def __init__(self):
        ...
        self.dividends = pd.read_sql_query(
            str(Dividends.objects.all().query), connection, index_col='id')
    ...
    def portfolio_contain(self, ticker=None, date=None):
        ...
        query = """SELECT buy.ticker, 
                ...
                FROM (select ticker, sum(amount) amount, sum(price * amount) total from public.web_transactions 
                    where operation = 'Покупка акций'
                    group by ticker) as buy 
                left join (
                    select ticker, sum(amount) amount from public.web_transactions 
                    where operation = 'Продажа акций'
    ...
    def load_Stock_time_siries(self):
        query = "SELECT id , ticker   FROM Web_Transactions where operation = 'Покупка акций'"
